=begin
#Live API Reference

#Reference for the Brightcove Live API, used to create and manage live streaming events.  For additional in-depth guides to features of the API, see the **[Support Site](https://support.brightcove.com/live-api)**.  **Base URL**: https://api.bcovlive.io/v1

The version of the OpenAPI document: 1.0.0

Generated by: https://openapi-generator.tech
OpenAPI Generator version: 4.0.0

=end

require 'date'

module BrightcoveLive
  class ApplicationApplicationAdConfiguration
    # The ad configuration description
    attr_accessor :ad_configuration_description

    # The ad configuration strategy (SingleAdResponse, or MultipleAdResponse)
    attr_accessor :ad_configuration_strategy

    # The ad configuration transforms
    attr_accessor :ad_configuration_transforms

    # The URL template for the ad server
    attr_accessor :ad_configuration_url_format

    # Key/value pairs for the ad server URL template
    attr_accessor :ad_configuration_variables

    # The ad configuration headers
    attr_accessor :headers

    # The ad configuration response type (Dfp, Vast, or SmartXML)
    attr_accessor :response_type

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'ad_configuration_description' => :'ad_configuration_description',
        :'ad_configuration_strategy' => :'ad_configuration_strategy',
        :'ad_configuration_transforms' => :'ad_configuration_transforms',
        :'ad_configuration_url_format' => :'ad_configuration_url_format',
        :'ad_configuration_variables' => :'ad_configuration_variables',
        :'headers' => :'headers',
        :'response_type' => :'response_type'
      }
    end

    # Attribute type mapping.
    def self.openapi_types
      {
        :'ad_configuration_description' => :'String',
        :'ad_configuration_strategy' => :'String',
        :'ad_configuration_transforms' => :'Array<ApplicationApplicationAdConfigurationAdConfigurationTransforms>',
        :'ad_configuration_url_format' => :'String',
        :'ad_configuration_variables' => :'Object',
        :'headers' => :'Object',
        :'response_type' => :'String'
      }
    end

    # Initializes the object
    # @param [Hash] attributes Model attributes in the form of hash
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `BrightcoveLive::ApplicationApplicationAdConfiguration` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BrightcoveLive::ApplicationApplicationAdConfiguration`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'ad_configuration_description')
        self.ad_configuration_description = attributes[:'ad_configuration_description']
      end

      if attributes.key?(:'ad_configuration_strategy')
        self.ad_configuration_strategy = attributes[:'ad_configuration_strategy']
      end

      if attributes.key?(:'ad_configuration_transforms')
        if (value = attributes[:'ad_configuration_transforms']).is_a?(Array)
          self.ad_configuration_transforms = value
        end
      end

      if attributes.key?(:'ad_configuration_url_format')
        self.ad_configuration_url_format = attributes[:'ad_configuration_url_format']
      end

      if attributes.key?(:'ad_configuration_variables')
        self.ad_configuration_variables = attributes[:'ad_configuration_variables']
      end

      if attributes.key?(:'headers')
        self.headers = attributes[:'headers']
      end

      if attributes.key?(:'response_type')
        self.response_type = attributes[:'response_type']
      end
    end

    # Show invalid properties with the reasons. Usually used together with valid?
    # @return Array for valid properties with the reasons
    def list_invalid_properties
      invalid_properties = Array.new
      if @ad_configuration_description.nil?
        invalid_properties.push('invalid value for "ad_configuration_description", ad_configuration_description cannot be nil.')
      end

      if @ad_configuration_strategy.nil?
        invalid_properties.push('invalid value for "ad_configuration_strategy", ad_configuration_strategy cannot be nil.')
      end

      if @ad_configuration_transforms.nil?
        invalid_properties.push('invalid value for "ad_configuration_transforms", ad_configuration_transforms cannot be nil.')
      end

      if @ad_configuration_url_format.nil?
        invalid_properties.push('invalid value for "ad_configuration_url_format", ad_configuration_url_format cannot be nil.')
      end

      if @ad_configuration_variables.nil?
        invalid_properties.push('invalid value for "ad_configuration_variables", ad_configuration_variables cannot be nil.')
      end

      if @headers.nil?
        invalid_properties.push('invalid value for "headers", headers cannot be nil.')
      end

      if @response_type.nil?
        invalid_properties.push('invalid value for "response_type", response_type cannot be nil.')
      end

      invalid_properties
    end

    # Check to see if the all the properties in the model are valid
    # @return true if the model is valid
    def valid?
      return false if @ad_configuration_description.nil?
      return false if @ad_configuration_strategy.nil?
      return false if @ad_configuration_transforms.nil?
      return false if @ad_configuration_url_format.nil?
      return false if @ad_configuration_variables.nil?
      return false if @headers.nil?
      return false if @response_type.nil?
      true
    end

    # Checks equality by comparing each attribute.
    # @param [Object] Object to be compared
    def ==(o)
      return true if self.equal?(o)
      self.class == o.class &&
          ad_configuration_description == o.ad_configuration_description &&
          ad_configuration_strategy == o.ad_configuration_strategy &&
          ad_configuration_transforms == o.ad_configuration_transforms &&
          ad_configuration_url_format == o.ad_configuration_url_format &&
          ad_configuration_variables == o.ad_configuration_variables &&
          headers == o.headers &&
          response_type == o.response_type
    end

    # @see the `==` method
    # @param [Object] Object to be compared
    def eql?(o)
      self == o
    end

    # Calculates hash code according to all attributes.
    # @return [Integer] Hash code
    def hash
      [ad_configuration_description, ad_configuration_strategy, ad_configuration_transforms, ad_configuration_url_format, ad_configuration_variables, headers, response_type].hash
    end

    # Builds the object from hash
    # @param [Hash] attributes Model attributes in the form of hash
    # @return [Object] Returns the model itself
    def self.build_from_hash(attributes)
      new.build_from_hash(attributes)
    end

    # Builds the object from hash
    # @param [Hash] attributes Model attributes in the form of hash
    # @return [Object] Returns the model itself
    def build_from_hash(attributes)
      return nil unless attributes.is_a?(Hash)
      self.class.openapi_types.each_pair do |key, type|
        if type =~ /\AArray<(.*)>/i
          # check to ensure the input is an array given that the attribute
          # is documented as an array but the input is not
          if attributes[self.class.attribute_map[key]].is_a?(Array)
            self.send("#{key}=", attributes[self.class.attribute_map[key]].map { |v| _deserialize($1, v) })
          end
        elsif !attributes[self.class.attribute_map[key]].nil?
          self.send("#{key}=", _deserialize(type, attributes[self.class.attribute_map[key]]))
        end # or else data not found in attributes(hash), not an issue as the data can be optional
      end

      self
    end

    # Deserializes the data based on type
    # @param string type Data type
    # @param string value Value to be deserialized
    # @return [Object] Deserialized data
    def _deserialize(type, value)
      case type.to_sym
      when :DateTime
        DateTime.parse(value)
      when :Date
        Date.parse(value)
      when :String
        value.to_s
      when :Integer
        value.to_i
      when :Float
        value.to_f
      when :Boolean
        if value.to_s =~ /\A(true|t|yes|y|1)\z/i
          true
        else
          false
        end
      when :Object
        # generic object (usually a Hash), return directly
        value
      when /\AArray<(?<inner_type>.+)>\z/
        inner_type = Regexp.last_match[:inner_type]
        value.map { |v| _deserialize(inner_type, v) }
      when /\AHash<(?<k_type>.+?), (?<v_type>.+)>\z/
        k_type = Regexp.last_match[:k_type]
        v_type = Regexp.last_match[:v_type]
        {}.tap do |hash|
          value.each do |k, v|
            hash[_deserialize(k_type, k)] = _deserialize(v_type, v)
          end
        end
      else # model
        BrightcoveLive.const_get(type).build_from_hash(value)
      end
    end

    # Returns the string representation of the object
    # @return [String] String presentation of the object
    def to_s
      to_hash.to_s
    end

    # to_body is an alias to to_hash (backward compatibility)
    # @return [Hash] Returns the object in the form of hash
    def to_body
      to_hash
    end

    # Returns the object in the form of hash
    # @return [Hash] Returns the object in the form of hash
    def to_hash
      hash = {}
      self.class.attribute_map.each_pair do |attr, param|
        value = self.send(attr)
        next if value.nil?
        hash[param] = _to_hash(value)
      end
      hash
    end

    # Outputs non-array value in the form of hash
    # For object, use to_hash. Otherwise, just return the value
    # @param [Object] value Any valid value
    # @return [Hash] Returns the value in the form of hash
    def _to_hash(value)
      if value.is_a?(Array)
        value.compact.map { |v| _to_hash(v) }
      elsif value.is_a?(Hash)
        {}.tap do |hash|
          value.each { |k, v| hash[k] = _to_hash(v) }
        end
      elsif value.respond_to? :to_hash
        value.to_hash
      else
        value
      end
    end
  end
end

...
module BrightcoveLive
  # Details of the connection information
  class RTMPoutConnectionInfo
    # Host URL
    attr_accessor :host
    ...
    # The application name in the path
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `BrightcoveLive::RTMPoutConnectionInfo` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BrightcoveLive::RTMPoutConnectionInfo`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'session_status')
    ...
    def valid?
      session_status_validator = EnumAttributeValidator.new('String', ["Active", "Inactive"])
    ...
    def hash
      [host, port, application, stream_name, session_status].hash
    end

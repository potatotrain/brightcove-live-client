...
module BrightcoveLive
  class CuePoint
    # The cuepoint insertion accuracy - may be segment or frame
    attr_accessor :accuracy

    # The cuepoint duration in seconds
    attr_accessor :duration

    # The cuepoint id
    attr_accessor :id

    # Time when the cue point was inserted in the stream
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `BrightcoveLive::CuePoint` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BrightcoveLive::CuePoint`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [accuracy, duration, id, inserted_at].hash
    end

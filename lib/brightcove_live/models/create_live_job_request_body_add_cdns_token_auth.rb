...
module BrightcoveLive
  ...
  class CreateLiveJobRequestBodyAddCdnsTokenAuth
    # Token authentication type - currently, the only supported value is `Akamai2.0`
    attr_accessor :auth_type

    # Your Akamai token auth password
    attr_accessor :key

    attr_accessor :media

    # Your Akamai token token name
    ...
    def self.openapi_types
      {
        :'auth_type' => :'String',
        :'key' => :'String',
        :'media' => :'CreateLiveJobRequestBodyAddCdnsTokenAuthMedia',
        :'token_name' => :'String'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `BrightcoveLive::CreateLiveJobRequestBodyAddCdnsTokenAuth` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BrightcoveLive::CreateLiveJobRequestBodyAddCdnsTokenAuth`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [auth_type, key, media, token_name].hash
    end

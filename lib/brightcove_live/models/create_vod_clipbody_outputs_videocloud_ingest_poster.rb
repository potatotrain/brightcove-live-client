...
module BrightcoveLive
  ...
  class CreateVodClipbodyOutputsVideocloudIngestPoster
    # URL for the video poster image
    attr_accessor :url

    # Height of the poster in pixels
    attr_accessor :height

    # Width of the poster in pixels
    attr_accessor :width
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `BrightcoveLive::CreateVodClipbodyOutputsVideocloudIngestPoster` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BrightcoveLive::CreateVodClipbodyOutputsVideocloudIngestPoster`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

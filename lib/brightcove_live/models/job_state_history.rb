...
module BrightcoveLive
  class JobStateHistory
    ...
    # Time when the transition to that state occurred in Epoch (UNIX) time
    attr_accessor :transition_timestamp
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `BrightcoveLive::JobStateHistory` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `BrightcoveLive::JobStateHistory`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'transition_timestamp')
        self.transition_timestamp = attributes[:'transition_timestamp']
      end
    end
    ...
    def valid?
      state_validator = EnumAttributeValidator.new('String', ["error", "standby", "waiting", "processing", "disconnected", "finishing", "finished", "cancelling", "cancelled", "failed", "waiting_finish_live", "creating_asset"])
    ...
    def state=(state)
      validator = EnumAttributeValidator.new('String', ["error", "standby", "waiting", "processing", "disconnected", "finishing", "finished", "cancelling", "cancelled", "failed", "waiting_finish_live", "creating_asset"])
    ...
    def ==(o)
      ...
      self.class == o.class &&
          state == o.state &&
          transition_timestamp == o.transition_timestamp
    end
    ...
    def hash
      [state, transition_timestamp].hash
    end
